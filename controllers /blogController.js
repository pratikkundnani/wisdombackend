...
function fileDownload(fileId) {
  return new Promise(async (resolve, reject) => {
    await axios.get("https://api.backblazeb2.com/b2api/v3/b2_authorize_account", {
      ...
      // }
      const resp = await axios.get("https://f005.backblazeb2.com/b2api/v3/b2_download_file_by_id?fileId=" + fileId, {
...
const blogController = {
  ...
  likeBlog: async (req, res) => {
    try {
      ...
      await blog.save();
      await user.save();
      res.status(200).json({ message: 'Blog post liked successfully', blog: blog });
    } catch (error) {
  ...
  unlikeBlog: async (req, res) => {
    try {
      ...
      await blog.save();
      await user.save();
      res.status(200).json({ message: 'Blog post unliked successfully', blog: blog });
    } catch (error) {
  ...
  searchBlogs : async (req, res) => {
    try {
      const query = req.oremquery.search;
